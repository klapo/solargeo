...
#--------------------------------------------------------------------
## Type declarations, keeping around for future conversion to cython
#    .. Scalar Arguments ..
#      INTEGER   YEAR, DAY
...
# Force right ascension between 0 and 2*pi
    inds = np.nonzero(DEN<0.0)
    RA[inds] += PI
    inds = np.nonzero((NUM<0.0) & (DEN >= 0.0))
    RA[inds] += TWOPI
...
# Put azimuth between 0 and 2*pi radians
    inds = np.nonzero( (np.sin(DEC)-np.sin(EL)*np.sin(LAT*RPD)>=0.) & (np.sin(AZ)<0.))
    AZ[inds] += TWOPI
...
#===================================================================
# Complete
    if np.min(EL)<-90.0 or np.max(EL)>90.0:
